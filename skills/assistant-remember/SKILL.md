...
2026-09-14 起按 `portrait/self.md` 的现行职责记录：交流、分析、学习、个人处境与助理行为校正。项目执行流水、部署状态、账号令牌、投递管道和工作待办不进入当前记忆；work 副系统与本助理无关，不登记为待设计事项，也不设计交接或回流。已有原始记录保留供定向回查，不从历史恢复工作职责。
...
1. 不能确认属于原话、认可、决定、报备、痕迹或归档的，一律打 〔AI〕，不许裸写。
...
`working.md` 保存近期交流、分析、学习和个人状态；`tracks.md` 保存长期学习、兴趣与生活线索。两者都由本 skill 维护，不承担工作项目管理。

轨道条目原地重写为当前状态，不打日期补丁；演变过程由 journal 承载，稳定模式由 `assistant-portrait` 沉淀到 portrait。

判断规则（决定一条内容写入 working 还是 tracks）：

1. 与当前职责相关、预计两周内会变化的状态写入 `working.md`；跨月的学习、兴趣、健康与生活线索写入 `tracks.md`。学习项目只保留入口、本人尝试与卡点，不带入运维状态。
...
- 做出路径选择或撤回旧判断。
- 产生可用于画像建模的高质量反例。
- 发生与交流、分析、学习或本系统维护有关的连续问答、方案比较、尝试与文件变更。
...
- **四标记恰好一个**：〔痕〕本次核验的文件或行为痕迹；〔报〕主人会话报备；〔决〕主人明确决定/声明；〔档〕历史回溯补录（仅补录时用）。日常记忆不自动运行跨仓库 Git 采集，不为完整性抓取工作流水；历史回填工具仅在明确的证据核对需求下使用，Python 通过 uv 运行。
