...
3. 读取 `/home/arcaneorion/user-memory/diary/` 全量（第一权重，主人手写原生数据）。注意：diary 是 AI 零接触区——助理只读取，绝不写入。
...
7. 如 portrait 信息不足或用户要求版本回溯，再读取 archive 最新 `portrait_snapshot`（`archive/v2/`、`archive/v3/` 等）；`archive/v1/` 只能作为 `legacy_prior` 低权重参考。archive 仅作为 fallback 或版本回溯材料。
...
| 文件/目录 | 职责 |
|---|---|
| `diary/` | 主人每日手写原生日记，第一权重，AI 零接触（助理严禁写入），由主人自己维护 |
